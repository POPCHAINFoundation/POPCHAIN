...
CzPCHWallet::CzPCHWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpch seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPCHSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPCHSeed_deprecated()) {
                    LogPrintf("%s: Updated zPCH seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpch seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpch wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzPCHWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPCHCount(nCountLastUsed);
    else if (!walletdb.ReadZPCHCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzPCHWallet::Lock()
{
    seedMaster = 0;
}

void CzPCHWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzPCHWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPCH(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzPCHWallet::LoadMintPoolFromDB()
{
...
void CzPCHWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzPCHWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzPCHWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpchTracker->HasPubcoinHash(pMint.first)) {
...
bool CzPCHWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPCH(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpchTracker which also adds to database
    pwalletMain->zpchTracker->Add(dMint, true);
...
void CzPCHWallet::SeedToZPCH(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzPCHWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzPCHWallet::GenerateDeterministicZPCH(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzPCHWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzPCHWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzPCHWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
