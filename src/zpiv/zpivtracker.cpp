...
CzPCHTracker::CzPCHTracker(std::string strWalletFile)
{
...
CzPCHTracker::~CzPCHTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzPCHTracker::Init()
{
...
bool CzPCHTracker::Archive(CMintMeta& meta)
{
...
bool CzPCHTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzPCHTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzPCHTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzPCHTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzPCHTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzPCHTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzPCHTracker::GetSerialHashes()
{
...
CAmount CzPCHTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzPCHTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzPCHTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzPCHTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzPCHTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzPCHTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzPCHTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzPCHTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzPCHTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzPCHTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzPCHTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzPCHWallet* zPCHWallet)
{
    bool iszPCHWalletInitialized = (NULL != zPCHWallet);
    CMintMeta meta;
    ...
    if (! iszPCHWalletInitialized)
        zPCHWallet = new CzPCHWallet(strWalletFile);
    meta.isSeedCorrect = zPCHWallet->CheckSeed(dMint);
    if (! iszPCHWalletInitialized)
        delete zPCHWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzPCHTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzPCHTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzPCHTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzPCHTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzPCHTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzPCHTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzPCHWallet* zPCHWallet = new CzPCHWallet(strWalletFile);
