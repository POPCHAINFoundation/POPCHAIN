...
#include "ui_zpchcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zpchTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zpchTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zPCH.";
            else if (!mint.isSeedCorrect)
                strReason = "The zPCH seed used to mint this zPCH is not the same as currently hold in the wallet";
            else
