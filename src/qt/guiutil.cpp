...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a PCH address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no POPCHAIN: URI
...
{
    // Convert popchain:// to popchain:
    //
    //    Cannot handle this later, because popchain:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open popchain.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "POPCHAIN.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for POPCHAIN.lnk
...
{
    return GetAutostartDir() / "popchain.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a popchain.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=POPCHAIN\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the popchain app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add popchain app to startup item list
