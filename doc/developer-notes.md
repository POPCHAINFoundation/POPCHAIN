...
- [Development guidelines](#development-guidelines)
    - [General Popchain Core](#general-popchain-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Popchain Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play PCHs (tPCH)" on the test network, if you
...
Popchain Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/popchain-project/popchain/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_popchain
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_popchain --log_level=test_suite
$ valgrind -v --leak-check=full src/popchaind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_popchain.coverage/index.html`.
```
...
- BitcoinMiner : Generates PCHs (if wallet is enabled).
...
However, in open source software such as Popchain Core, where everyone uses
...
pay attention to for reviewers of Popchain Core code.

General Popchain Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Popchain Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Popchain Core so that they can be integrated
...
file descriptors it will cause problems with POPCHAIN's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:POPCHAIN-Project/POPCHAIN.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `popchain-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `popchain-cli` and the GUI debug console use this table to determine how to
