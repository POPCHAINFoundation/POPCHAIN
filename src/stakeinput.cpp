...
// The zPCH block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zPCH is a hash of the serial
...
{
    //Create an output returning the zPCH that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZPCHOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zPCH output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zPCH", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZPCHOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zPCH output", __func__);
...
{
    CzPCHTracker* zpchTracker = pwallet->zpchTracker.get();
    CMintMeta meta;
    if (!zpchTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zpchTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a PCH stake is the outpoint
