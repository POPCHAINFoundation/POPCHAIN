...
public:
    ...
    bool WriteZPCHSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZPCHSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZPCHSeed_deprecated(uint256& seed);
    bool EraseZPCHSeed();
    bool EraseZPCHSeed_deprecated();

    bool WriteZPCHCount(const uint32_t& nCount);
    bool ReadZPCHCount(uint32_t& nCount);
