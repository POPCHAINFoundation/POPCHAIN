...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "popchain" is a composite category enabling all POPCHAIN-related debug output
            if (ptrCategory->count(string("popchain"))) {
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "POPCHAIN";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "popchain.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty popchain.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override popchain.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "popchaind.pid"));
